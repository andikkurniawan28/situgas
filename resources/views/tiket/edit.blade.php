                    ...
                    <form action="{{ route('tiket.update', $tiket->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <select name="klien_id" id="klien_id"
                                ...
                                @foreach ($kliens as $klien)
                                    <option value="{{ $klien->id }}"
                                        {{ (old('klien_id') ?? $tiket->klien_id) == $klien->id ? 'selected' : '' }}>
                            ...
                            <select name="jenis_tiket_id" id="jenis_tiket_id"
                                ...
                                @foreach ($jenis_tikets as $jenis)
                                    <option value="{{ $jenis->id }}"
                                        {{ (old('jenis_tiket_id') ?? $tiket->jenis_tiket_id) == $jenis->id ? 'selected' : '' }}>
                                        {{ $jenis->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mt-3">
                            ...
                            <input type="text" name="judul" id="judul"
                                ...
                                value="{{ old('judul', $tiket->judul) }}" required>
                            @error('judul')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mt-3">
                            ...
                            <textarea name="keterangan" id="keterangan" class="form-control @error('keterangan') is-invalid @enderror"
                                rows="4" required>{{ old('keterangan', $tiket->keterangan) }}</textarea>
                            @error('keterangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select name="didelegasikan_ke" id="didelegasikan_ke"
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ (old('didelegasikan_ke') ?? $tiket->didelegasikan_ke) == $user->id ? 'selected' : '' }}>
